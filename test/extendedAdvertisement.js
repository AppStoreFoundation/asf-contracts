...
var ExtendeAdvertisementFinance = artifacts.require("./ExtendedFinance.sol");
...
contract('ExtendedAdvertisement', function(accounts) {
  beforeEach('Setting ExtendedAdvertisement test...',async () => {
	...
	it('should emit an event when PoA is received', function () {
		return addInstance.bulkRegisterPoA(0x02,0x02,0x02,4).then( instance => {
	...
	it('should set the Campaign validity to false when the remaining budget is smaller than the price', function () {
		return addInstance.bulkRegisterPoA(0x0,0x0,0x0,4).then( instance => {
			fail("Needs implementation");
		});
	});

	it('should registerPoA and transfer the equivalent to one installation to the trusted party balance on finance contract', async function () {
		var userInitBalance = await TestUtils.getBalance(accounts[0]);
